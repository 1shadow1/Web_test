...
export class CubismMotionManager extends CubismMotionQueueManager {
  ...
  _currentPriority: number; // 現在再生中のモーションの優先度   当前播放的运动优先级
  _reservePriority: number; // 再生予定のモーションの優先度。再生中は0になる。モーションファイルを別スレッドで読み込むときの機能。要播放的运动的优先级。播放中为0。在其他线程中导入运动文件时的功能。
}
